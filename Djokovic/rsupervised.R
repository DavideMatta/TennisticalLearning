# whole dataset odds accuracy 0.8579041 
...
linreg2 = lm(Win ~ Log_Rank_Djo + Log_Rank_Opp + Five_Set + Clay + Opp_Court_Perc + Pre_Opp + Pre5_Opp, data = train)
summary(linreg2)
vif(linreg2)
check_model(linreg2)
mod_graph = check_model(linreg2)
plot(mod_graph)[[5]]
...
logistic = glm(Win ~ .- Hard - Rank_Djo - Rank_Opp - Odd_Pred, data = train, family = binomial(link = 'logit'))
tidy(logistic)
summary(logistic)
...
# best subset selection
logistic2 = glm(Win ~ Log_Rank_Djo + Log_Rank_Opp + Five_Set + Clay + Opp_Court_Perc + Pre_Opp + Pre5_Opp, data = train, family = binomial(link = 'logit'))
tidy(logistic2)
summary(logistic2)
...
ols_robust = rlm(Win ~ .  - Hard - Rank_Djo - Rank_Opp - Odd_Pred, data = train, se_type = "HC2")
summary(ols_robust)
...
ols_robust2 = rlm(Win ~ Log_Rank_Djo + Log_Rank_Opp + Five_Set + Clay + Opp_Court_Perc + Pre_Opp + Pre5_Opp, data = train, se_type = "HC2")
summary(ols_robust2)
...
plot(predict(logistic,newdata = sdf, type = "response"))
...
confrontation = data.frame(
  ...
  Logit = predict(logistic,newdata = sdf, type = "response"),
...
confrontation_test = data.frame(
  ...
  Logit = predict(logistic,newdata = test, type = "response"),
...
rg = read.csv('../data/djo_fdf_rg.csv')
rg

plot(predict.lm(linreg2,newdata = rg))
plot(predict(logistic,newdata = rg, type = "response"))
