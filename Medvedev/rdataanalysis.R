...
sdf = df[, !(names(df) %in% c('Odd_Med', 'Odd_Opp', 'Odd_Pred', 'Odd_Acc'))]
...
cor(df$Rank_Med, df$Win)
cor(df$Log_Rank_Med, df$Win)
ggplot(df, aes(x = as.factor(Win), y = Log_Rank_Med)) + geom_boxplot() 
ggplot(df, aes(x = Rank_Med)) + geom_histogram(bins = 30) 
ggplot(df, aes(x = Log_Rank_Med, color = as.factor(Win), fill = as.factor(Win))) + geom_density(linewidth = 0.75, alpha = 0.5) 
...
cor(df$H2H_Med, df$Win)
ggplot(df, aes(x = as.factor(Win), y = H2H_Med)) + geom_boxplot() 
ggplot(df, aes(x = (H2H_Med))) + geom_histogram(bins = 50) 
ggplot(df, aes(x = H2H_Med, color = as.factor(Win), fill = as.factor(Win))) + geom_density(linewidth = 0.75, alpha = 0.5) 


cor(df$Pre_Med, df$Win)
ggplot(df, aes(x = as.factor(Pre_Med), fill = as.factor(Win))) + geom_bar()
ggplot(df, aes(x = as.factor(Pre_Med), fill = as.factor(Win))) + geom_bar(position = 'fill')

cor(df$Pre5_Med, df$Win)
ggplot(df, aes(x = as.factor(Pre5_Med), fill = as.factor(Win))) + geom_bar()
ggplot(df, aes(x = as.factor(Pre5_Med), fill = as.factor(Win))) + geom_bar(position = 'fill')
