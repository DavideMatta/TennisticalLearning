#whole dataset odds accuracy 0.7402597  
...
linreg2 = lm(Win ~ Log_Rank_Med + Log_Rank_Opp + Clay + Opp_Court_Perc + Pre_Med, data = train)
summary(linreg2)
vif(linreg2)
check_model(linreg2)
...
logistic = glm(Win ~ .- Hard - Rank_Med - Rank_Opp - Odd_Pred, data = train, family = binomial(link = 'logit'))
tidy(logistic)
summary(logistic)
...
# best subset selection
logistic2 = glm(Win ~ Log_Rank_Med + Log_Rank_Opp + Clay + Opp_Court_Perc + Pre_Med, data = train, family = binomial(link = 'logit'))
tidy(logistic2)
...
ols_robust = rlm(Win ~ .  - Hard - Rank_Med - Rank_Opp - Odd_Pred, data = train, se_type = "HC2")
summary(ols_robust)
...
ols_robust2 = rlm(Win ~ Log_Rank_Med + Log_Rank_Opp + Clay + Opp_Court_Perc + Pre_Med, data = train, se_type = "HC2")
summary(ols_robust2)
...
plot(predict.lm(linreg,newdata = sdf))
plot(predict(logistic2,newdata = sdf, type = "response"))
plot(predict.lm(ols_robust,newdata = sdf))


confrontation = data.frame(
  True = df$Win,
  Odd = df$Odd_Pred,
  Ols = predict.lm(linreg,newdata = sdf),
  Logit = predict(logistic2,newdata = sdf, type = "response"),
  Robust = predict.lm(ols_robust,newdata = sdf)
)
head(confrontation)
...
confrontation_test = data.frame(
  True = test$Win,
  Odd = test$Odd_Pred,
  Ols = predict.lm(linreg,newdata = test),
  Logit = predict(logistic2,newdata = test, type = "response"),
  Robust = predict.lm(ols_robust,newdata = test)
)
...
rg = read.csv('../data/med_fdf_rg.csv')
rg

plot(predict.lm(linreg,newdata = rg))
plot(predict(logistic2,newdata = rg, type = "response"))
plot(predict.lm(ols_robust,newdata = rg))
...
confrontation_rg = data.frame(
  ...
  Logit = predict(logistic,newdata = rg, type = "response"),
  Robust = predict.lm(ols_robust2,newdata = rg)
)

head(confrontation_rg)
